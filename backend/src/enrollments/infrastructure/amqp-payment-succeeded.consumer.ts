...
@Injectable()
export class AmqpPaymentSucceededConsumer
  implements OnModuleInit, OnModuleDestroy
{
  private readonly logger = new Logger(AmqpPaymentSucceededConsumer.name);
  ...
  constructor(
    ...
    private readonly processPaymentSucceeded: ProcessPaymentSucceededService,
  ) {
    ...
    this.queue = config.getOrThrow<string>(
      'rabbitmq.enrollmentsPaymentSucceededQueue',
    );
  ...
  async onModuleInit(): Promise<void> {
    if (!this.enabled) {
      this.logger.log('Enrollment payment.succeeded consumer is disabled');
      return;
    }
  ...
  private async handleMessage(message: ConsumeMessage | null): Promise<void> {
    ...
    try {
      const event = this.parsePaymentSucceededEvent(message);
      await this.processPaymentSucceeded.process(event);
      ...
      this.logger.warn(
        `Failed to consume payment.succeeded message: ${consumerError.message}`,
      );
      this.channel.nack(message, false, true);
    }
  }

  private parsePaymentSucceededEvent(
    ...
    try {
      ...
      throw new NonRetryableMessageError(
        'Invalid JSON in payment.succeeded message',
      );
    }

    if (!isPaymentSucceededEventV1(parsed)) {
      throw new NonRetryableMessageError('Invalid payment.succeeded message');
    }
...
function isPaymentSucceededEventV1(
  event: unknown,
  ...
  const candidate = event as Partial<PaymentSucceededEventV1>;
