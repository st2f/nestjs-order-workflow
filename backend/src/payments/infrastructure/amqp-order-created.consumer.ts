...
@Injectable()
export class AmqpOrderCreatedConsumer implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(AmqpOrderCreatedConsumer.name);
  ...
  constructor(
    ...
    private readonly processOrderCreated: ProcessOrderCreatedService,
  ) {
    ...
    this.queue = config.getOrThrow<string>(
      'rabbitmq.paymentsOrderCreatedQueue',
    );
  ...
  async onModuleInit(): Promise<void> {
    if (!this.enabled) {
      this.logger.log('Payment order.created consumer is disabled');
      return;
    }
  ...
  private async handleMessage(message: ConsumeMessage | null): Promise<void> {
    ...
    try {
      const event = this.parseOrderCreatedEvent(message);
      await this.processOrderCreated.process(event);
      this.channel.ack(message);
    } catch (error) {
      const consumerError =
        error instanceof Error ? error : new Error(String(error));

      if (consumerError instanceof NonRetryableMessageError) {
      ...
      this.logger.warn(
        `Failed to consume order.created message: ${consumerError.message}`,
      );
      this.channel.nack(message, false, true);
    }
  }

  private parseOrderCreatedEvent(message: ConsumeMessage): OrderCreatedEventV1 {
    let parsed: unknown;
    ...
    try {
      ...
      throw new NonRetryableMessageError(
        'Invalid JSON in order.created message',
      );
    }

    if (!isOrderCreatedEventV1(parsed)) {
      throw new NonRetryableMessageError('Invalid order.created message');
    }
...
function isOrderCreatedEventV1(event: unknown): event is OrderCreatedEventV1 {
  if (!event || typeof event !== 'object') {
    return false;
  }

  const candidate = event as Partial<OrderCreatedEventV1>;

  return (
    candidate.eventType === 'order.created' &&
    candidate.version === 1 &&
    ...
    typeof candidate.data.courseId === 'string' &&
    typeof candidate.data.amount === 'string'
  );
}
